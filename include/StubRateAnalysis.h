...
#ifndef StubRateAnalysis_h
#define StubRateAnalysis_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   StubRateAnalysis(TTree *tree=0);
   virtual ~StubRateAnalysis();
   virtual Int_t    Cut(Long64_t entry);
   virtual Int_t    GetEntry(Long64_t entry);
   virtual Long64_t LoadTree(Long64_t entry);
   virtual void     Init(TTree *tree);
   void     Loop(TString, float, TString);
   virtual Bool_t   Notify();
   virtual void     Show(Long64_t entry = -1);
};

StubRateAnalysis::StubRateAnalysis(TTree *tree) : fChain(0) 
{
...
StubRateAnalysis::~StubRateAnalysis()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t StubRateAnalysis::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t StubRateAnalysis::LoadTree(Long64_t entry)
{
...
void StubRateAnalysis::Init(TTree *tree)
{
...
Bool_t StubRateAnalysis::Notify()
{
...
void StubRateAnalysis::Show(Long64_t entry)
{
...
Int_t StubRateAnalysis::Cut(Long64_t entry)
{
...
#endif // #ifdef StubRateAnalysis_cxx
