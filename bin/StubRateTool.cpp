#define StubRateAnalysis_cxx
#include "L1Trigger/StubStudies/include/StubRateAnalysis.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include "TRandom.h"
...
static void show_usage(std::string name){
  std::cerr << "Usage: " << name << " <options>"
	    << "Options:\n"
            << "\t-i The name of the input directory. If blank, defaults to ./output.root\n"
            << "\t-o The name of the output file. If blank, defaults to ./analysisOutput.root\n"
            << "\t-p --particle Sets which particles are analysed. Options: all, mu, ele, pion, tau. Default is all.\n"
            << "\t-n The number of entries to run over.\n"
...
int main(int argc, char* argv[]){
    ...
    for (int i = 1; i < argc; ++i){
      ...
      else if ( (arg=="-p") || (arg == "--particle") ) { // using this option sets which particles are analysied
        if (i+1 < argc){
          particleName = argv[++i];
        }
      }
...
void StubRateAnalysis::Loop(TString fname, float Nin, TString pname){
...
//[endcap][disk][ring][module phi index]
  ...
  for (unsigned long i=0;i<regions.size();++i){
    for (unsigned long j=0;j<layers.size();++j){
      for (unsigned long k=0;k<channels.size();++k){
        for (unsigned long l=0;l<vars.size();++l){
  ...
  for (unsigned long i=0;i<regions.size();++i){
    for (unsigned long j=0;j<channels.size();++j){
      for (unsigned long l=0;l<category.size();++l){
...
// efficiency variables
  ...
  for (unsigned long i=0;i<regions.size();++i){
    for (unsigned long j=0;j<layers.size();++j){
      for (unsigned long k=0;k<channelsEff.size();++k){
        for (unsigned long l=0;l<varsEff.size();++l){
...
//Window tuning variables
  ...
  for (int i=0;i<int(regions.size());++i){
    for (int j=0;j<int(layers.size());++j){
      for (int k=0;k<int(channelsSW.size());++k){
...
//tracking particle vattriables
  ...
  for (unsigned long l=0;l<TPvars.size();++l){
    name<<"TP_"<<TPvars[l];
...
//  int ch = 0;
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    for (unsigned long j=0;j<regions.size();++j){
      for (unsigned long k=0;k<layers.size();++k){
	for (unsigned long l=0;l<100;++l){
	  for (unsigned long m=0;m<100;++m){
...
//SW histograms
  ...
  for (unsigned long j=0;j<regions.size();++j){
    for (unsigned long k=0;k<layers.size();++k){
      for (unsigned long l=0;l<vars.size()-1;++l){
...
//two dimensional plots
  ...
  for (unsigned long k=0;k<Endcap_2D.size();++k){
  ...
  for (unsigned long k=0;k<TPHists.size();++k){
